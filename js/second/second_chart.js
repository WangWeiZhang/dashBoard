var secondChart=(function(){
	...
	function lineChart(data){
		...
		var xTransfer;  //svg横方向偏移量
		var tip_y1  //case 的纵坐标的位置
		var tip_y2; //total 的纵坐标的位置
		var tip_y3; //unsolved 纵坐标提示的纵坐标位置
		var xaxi_y; //横坐标提示纵向位置
		var fontSize; //字体大小
		...
		//添加g
		var g = svg.append("g").attr("transform","translate("+xTransfer+","+height/6+")");  
		//获取折线图数据
		var lineArr = data.data.solvingOption;
		var lineArr1 = data.data.repeatOption;
		
		//获取x轴上的柱状图的最大值
		//solved 柱形图数组中最大值/2 然后将其四舍五入 
		...
		//x轴的比例尺   //0,1,2,3,4,5,6  //0 - 415.4812
		...
		//定义一些变量
		//矩形之间的空白
		var rectPadding = width/(size*2);  
		//字体大小
		var week13FontSize = "0.44rem";
		if($("#dataType").val() == 1){
			rectPadding = width/(size*1.8);
			week13FontSize = "0.4rem";
		}
		
		//矩形的宽度
		var rectWidth = xScale.rangeBand() - rectPadding;  
		...
		var line_color1 = defs.append("defs").append("linearGradient")
			...
			//添加横坐标提示
			textExp.selectAll(".xtext").data(xExp)  
						.enter()        
						.append("g")
						.append("text")
						.text(function(d , i){
					    ...
					    .style("fill","#fff")
						.style("font-size",week13FontSize)
			...
			//添加纵坐标左上提示
			textExp.selectAll(".y1text").data(y1Exp) 
			.enter()        
			.append("text")
			.text(function(d){
				if(d>=1000&&d<1000000){
			...
			//添加纵坐标右边提示
			textExp.selectAll(".y2text").data(y2Exp) 
			.enter()
			.append("text")
		  ...
		  var line_path = g.append("g");
		   	...
		   	//添加黄色圆圈
				var circle = g.append("g");
					circle.selectAll("circle").data(lineArr)  
							.enter()        
							.append("circle")
							...
							.attr("r", 3);
				//添加白色圆圈
				   circle.selectAll("circle1").data(lineArr1) 
							.enter()        
							.append("circle")
							.attr("cx",function(d,i){ return xScale(i)+rectPadding+rectWidth/2;})
							.attr("cy",function(d){return yScale1(d);} )
	...
	function drawAll(data, id,color,xExplain){
		...
		var xaxi_x; //定义x轴比例尺时使用
		var yaxi_y; //定义y轴比例尺时使用
		var tra_y; //画布纵向偏移量
		var posi_y;//纵坐标提示的y的偏移量
		var posi_x;//纵坐标提示的x的偏移量
		var tra_x;//画布横向偏移量
		if(window.screen.availWidth <= 1366){
			xaxi_x = 45;
			tra_x = 35;
		...
		g.append("path").datum(dataArr)
			...
			.attr("opacity",0.1);
        //添加纵坐标
		g.append("g").selectAll("line").data(yArr)
				.enter()        
				.append("line")
		...
		//添加横坐标提示
		g.append("g").selectAll("text").data(xArr)  
				.enter()        
				.append("text")
				.text(function(d , i){
					if(xArr.length == 24){
		...
		//添加纵坐标提示
		g.append("g").selectAll("text").data(yArr)  
				.enter()        
				.append("text")
				.text(function(d){
					if(d>=1000 && d<1000000 ){
