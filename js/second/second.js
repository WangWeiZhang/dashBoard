var second = (function() {
	...
	function change_type(data){
		var type = $("#dataType").val(data);//获取隐藏域中的value值（用来确定4个不同的纬度请求不同的参数）
    	var title = "";
    	...
    	konwledge_graph(configpath);
    	custome_ermotion(configpath);
		useing_counts(configpath); 
		problemAnalysis(configpath);
		knowladgeAnalysis(configpath);
		multiMediaChat(configpath);
		getTitleData(configpath);
		analysis(configpath);
	 ...
	 * secondChart.drawAll（）调用second_charts.js文件中的  secondChart对象下的drawAll方法 并且将 存储起来的json对象传递过去
	 * */
	function knowledgeGraphData(data){
		...
		var json ={
			...
			$("#wave_div").html("");
			secondChart.drawAll(json, "wave_div","#fff","time");
	}
	function getCircleData(nums,jsonData,midu){
		...
		$.getJSON("json/second/circle.json", function(data) {
			...
			secondChart.usingChart(jsonData , midu);
		});
	}
	// useing counts
...
//				
				if(minLength.length == maxLength.length){
					...
					for(var j = 0;j<minLength.length-1;j++){
						yushu = yushu+"0";
					}
				}
	...
	function knowladgeAnalysis(config) {
		...
		var options = {
			...
			success : function(data) {
					...
					var i = 0;
					replace();
					list = setInterval(replace,8000);//ajax请求一次 8秒调用一次这个函数 
					function replace(){
						var html="";
                        $.each(data.data.option, function(index,val) {
...
//					var MaxWidth_output = Math.max.apply(null , NumWidth_output)/100;
	...
	function getTitleData(config) {
		...
		var options = {
			url : config.basePath + "/second/getTitleData",
			type : 'post',
			data : param,
			async: true,
			dataType : "json",
	...
	*/
	function analysis(config) {
		var s2Linebar;
		clearInterval(s2Linebar);
		var param = {
			"dataType" : $("#dataType").val()
		}
		var echarts_data = [];
		var options = {
			url : config.basePath + "/second/analysis",
			type : 'post',
			data : param,
			async: true,
			dataType : "json",
			success : function(data) {
				secondChart.lineChart(data);
				s2Linebar = setInterval(function (){secondChart.lineChart(data)},37*1000);
		    }
		}    
		$.ajax(options);
